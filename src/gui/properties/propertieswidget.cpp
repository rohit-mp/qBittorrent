    ...
    case PropTabBar::MainTab:
        {
            ...
            m_ui->labelUpLimitVal->setText(m_torrent->uploadLimit() <= 0
                    ? C_INFINITY
                    : Utils::Misc::friendlySpeedUnit(m_torrent->uploadLimit(), speedUnitType, useDecimalPrefixes));

            m_ui->labelDlLimitVal->setText(m_torrent->downloadLimit() <= 0
                    ? C_INFINITY
                    : Utils::Misc::friendlySpeedUnit(m_torrent->downloadLimit(), speedUnitType, useDecimalPrefixes));
            ...
            const QString dlAvg = Utils::Misc::friendlySpeedUnit(
                    (m_torrent->totalDownload() / ((dlDuration == 0) ? -1 : dlDuration))
                    , speedUnitType, useDecimalPrefixes);
            m_ui->labelDlSpeedVal->setText(tr("%1 (%2 avg.)", "%1 and %2 are speed rates, e.g. 200KiB/s (100KiB/s avg.)")
                .arg(Utils::Misc::friendlySpeedUnit(
                        m_torrent->downloadPayloadRate(), speedUnitType, useDecimalPrefixes), dlAvg));

            const qlonglong ulDuration = m_torrent->activeTime();
            const QString ulAvg = Utils::Misc::friendlySpeedUnit(
                    (m_torrent->totalUpload() / ((ulDuration == 0) ? -1 : ulDuration))
                    , speedUnitType, useDecimalPrefixes);
            m_ui->labelUpSpeedVal->setText(tr("%1 (%2 avg.)", "%1 and %2 are speed rates, e.g. 200KiB/s (100KiB/s avg.)")
                .arg(Utils::Misc::friendlySpeedUnit(
                        m_torrent->uploadPayloadRate(), speedUnitType, useDecimalPrefixes), ulAvg));
